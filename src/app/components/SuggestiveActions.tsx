        ...
        {categories.map(cat => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-3.5 py-2 rounded-full text-sm transition-colors ${isActive ? 'bg-[var(--surface-2)]' : 'bg-transparent hover:bg-[var(--surface-2)]'}`}
              style={{
      ...
      {activeCat && (
        ...
        >
          ...
          {activeCat.prompts.map((prompt, i) => (
            <button
              key={i}
              className="flex items-center justify-between w-full px-4 py-3.5 text-left text-sm transition-colors hover:bg-[var(--surface-2)]"
              style={{
                borderBottom: i < activeCat.prompts.length - 1 ? '1px solid var(--border-subtle)' : 'none',
                color: 'var(--text-primary)',
              }}
              onMouseEnter={() => onHoverPrompt(prompt.query)}
              onMouseLeave={() => onHoverPrompt('')}
              onClick={() => {
                setActiveId(null);
                onHoverPrompt('');
